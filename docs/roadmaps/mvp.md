...
| **GS**   | Rule L3–L6 chain complete; writable-ideas + thesis summary overlays done | 2nd domain chain (2GS.7), develop all inklings to L3 (2NA.2) | —          |
| **UI**   | StatusBar, ChoiceList, save/load UI, tabbed summary overlay (held + ideas + thesis) | Thesis panel (2UI.2) | —                            |
...
- [x] 2GS.5. Display thesis summary (written ideas, dominant domains, discipline name) in UI — unified tabbed SummaryOverlay with Thesis tab (written ideas, per-domain orthodoxy bars, discipline subtitle); opens via `T` shortcut or StatusBar `⁋` button
- [x] 2GS.2. Implement writable-ideas UI panel (display held ideas, highlight writable, show level/orthodoxy) — SummaryOverlay Ideas tab (writable L3+ ideas); opens via `I`/`T` shortcuts or StatusBar `☉`/`⁋` buttons; tab-switching from either shortcut without closing
- [x] 2UI.1. Idea inventory panel — see held ideas, their level, and whether writable — added Held tab to SummaryOverlay showing L1 observations and L2 inklings grouped by level with per-domain orthodoxy bars; opens via `H` shortcut or StatusBar `◧` button; arrow keys cycle Held · Ideas · Thesis
