...
- **TypeScript** (src/lib/game/) — idea system, inventory, orthodoxy calculation
- **Bridge** (src/lib/engine/idea-bridge.ts) — binds TypeScript functions as Ink EXTERNAL functions
- **Engine** (src/lib/engine/story.svelte.ts) — loads compiled Ink JSON, manages story state with Svelte 5 runes
...
| `src/lib/engine/` | Ink runtime, bridge, story state |
...
- **EXTERNAL functions**: declared in `story/IdeaSystem.ink`, bound in `idea-bridge.ts`. Ink only supports string and number return values.
...
- **Catalog registration**: `registerAllIdeas()` and `registerAuthoredRecipes()` are called in `loadStory()` before the story begins.
